...
APP_VERSION = "5.2 Nordwerk Pro · HTML Fix"
...
def smtp_password():
    return os.getenv("HANDWERK_SMTP_PASSWORD", "")


def smtp_ready():
    return all([
        text(einstellungen.get("smtp_host")),
        text(einstellungen.get("smtp_user")),
        text(einstellungen.get("smtp_sender")),
        smtp_password(),
    ])
...
def email_senden(empfaenger, betreff, inhalt, pdf_bytes=None, pdf_name=None):
    ...
    try:
        ...
        host = text(einstellungen.get("smtp_host"))
        port = int(number(einstellungen.get("smtp_port"), 587))
        user = text(einstellungen.get("smtp_user"))
        pwd = smtp_password()
...
if st.session_state.page=="dashboard":
    ...
    test_empfaenger=text(einstellungen.get("smtp_sender"))
    ...
    with st.form("settings"):
        ...
        st.caption("SMTP-Passwort nicht hier speichern. Setze es in PowerShell als Umgebungsvariable HANDWERK_SMTP_PASSWORD.")
